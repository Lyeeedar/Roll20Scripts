    ...
    try {
        ...
        for (var i = 0; i < attacks.length; i++) {
            ...
            for (var c = 0; c < count; c++) {
                ...
                var atk = "<td>" + hit + " hit [[1d20" + hit + "cs>" + critvalue + "]]</td><td>dam [[" + damage + "]]";
                ...
                atkTemplate += "<tr><td>crit hit [[1d20" + hit + "cs>" + critvalue + "]]</td><td>crit dam [[" + damage + "*" + critMult.substr(1) + "]]</td></tr>";
            }
            sendChat("gmReminder", "/w gm <div class='sheet-rolltemplate-default'><table><caption>" + attack + "</caption> " + atkTemplate + "</table></div>");
        }
    } catch(err) {
        sendChat("gmReminder", "/w gm Failed to execute attack '" + rawattack + "' due to " + err.message);
    }
};
...
gmReminders.BuildAttackButtons = function(attackraw) {
    ...
    for (var i = 0; i < attacks.length; i++) {
        ...
        output +=  "  <a style='background:transparent; color:Red; padding:0; padding-right:5px' title='" + attack + "' href='!doattack " + attack + "'>" + shortname + "</a>";
    }
    ...
    for (var index = 0; index < lines.length; index++) {
        ...
        if (line.startsWith("Tactics")) {
            ...
            if (line.startsWith("Melee")) {
                var attackButton = gmReminders.BuildAttackButtons(line.replace("Melee ", ""));
                message += "<li><div align='left' style='clear:both'>Melee:" + attackButton + "</div></li>";
            }
        
            if (line.startsWith("Ranged")) {
                var attackButton = gmReminders.BuildAttackButtons(line.replace("Ranged ", ""));
                message += "<li><div align='left' style='clear:both'>Ranged:" + attackButton + "</div></li>";
            }
